                    ...
                    VStack(alignment: .leading, spacing: layout.groupSpacing) {
                        ...
                        ShellCard {
                            VStack(alignment: .leading, spacing: 8) {
                                ...
                                Text(t("ClawNest will refresh known Claws and the latest runtime snapshot after this modal closes.", "关闭这个弹窗后，ClawNest 会刷新已知 Claw 列表和最新运行时状态。"))
                                    .font(.footnote)
    ...
    private var headerBody: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(t("New Claw", "新建 Claw"))
                ...
                .foregroundStyle(.white)
            Text(t("Keep the main workspace stable while this modal handles environment checks, installation, failure messages, and the next setup step.", "让主工作区保持稳定，把环境检测、安装过程、失败提示和下一步引导都收进这个弹窗里。"))
                .font(.subheadline)
